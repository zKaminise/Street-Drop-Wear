...
import { ChevronRight, MapPin, Plus, Edit2, Trash2, Star, X, Check, AlertCircle } from 'lucide-react'
import { useAuthStore } from '@/lib/store'

type Address = {
  id: string; label: string; zipCode: string; street: string; number: string
  complement?: string; district: string; city: string; state: string; isDefault: boolean
}

const INITIAL: Omit<Address, 'id'> = {
  label: 'Casa', zipCode: '', street: '', number: '', complement: '', district: '', city: '', state: 'SP', isDefault: false,
}

const STATES = ['AC','AL','AM','AP','BA','CE','DF','ES','GO','MA','MG','MS','MT','PA','PB','PE','PI','PR','RJ','RN','RO','RR','RS','SC','SE','SP','TO']
const INPUT = 'w-full bg-black/40 border border-white/10 text-white placeholder-white/20 px-3 py-2 text-sm focus:outline-none focus:border-brand-red/60'
...
export default function EnderecosPage() {
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await fetch('/api/customer/addresses').then(r => r.json())
  ...
  function openEdit(a: Address) {
    setEditing(a)
    setForm({ label: a.label, zipCode: a.zipCode, street: a.street, number: a.number, complement: a.complement ?? '', district: a.district, city: a.city, state: a.state, isDefault: a.isDefault })
    setError('')
    setShowModal(true)
  }
      ...
      <div className="container-brand py-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-brand-red text-xs font-bold uppercase tracking-[0.3em]">Entrega</span>
              <h1 className="heading-display text-[clamp(2rem,4vw,3rem)] text-brand-white mt-1">ENDEREÇOS</h1>
            </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <MapPin size={48} className="text-brand-gray-text mx-auto mb-4" strokeWidth={1} />
            <p className="text-brand-gray-text mb-6">Nenhum endereço cadastrado.</p>
            <button onClick={openNew} className="btn-primary">Adicionar endereço</button>
          </div>
        ) : (
          <div className="space-y-4">
            {addresses.map((addr, i) => (
              <motion.div
                ...
                className={`bg-brand-graphite border p-5 flex items-start justify-between gap-4 ${addr.isDefault ? 'border-brand-red/30' : 'border-white/5'}`}
              >
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  <MapPin size={18} className="text-brand-red mt-0.5 flex-shrink-0" />
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-bold text-brand-white uppercase tracking-wide">{addr.label}</p>
                      {addr.isDefault && (
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => openEdit(addr)} className="p-1.5 text-white/30 hover:text-white transition-colors cursor-pointer">
                    <Edit2 size={14} />
                  </button>
                  <button onClick={() => handleDelete(addr.id)} className="p-1.5 text-white/30 hover:text-red-400 transition-colors cursor-pointer">
                    <Trash2 size={14} />
                  </button>
                </div>
              </motion.div>
          ...
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between p-5 border-b border-white/10">
                <h2 className="text-white font-bold">{editing ? 'Editar Endereço' : 'Novo Endereço'}</h2>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-xs text-white/50 uppercase tracking-wider mb-1 block">CEP *</label>
                    <input value={form.zipCode} onChange={e => setForm(p => ({ ...p, zipCode: e.target.value }))} className={INPUT} placeholder="00000-000" />
                  </div>
                  <div className="col-span-2">
                    <label className="text-xs text-white/50 uppercase tracking-wider mb-1 block">Rua *</label>
                    <input value={form.street} onChange={e => setForm(p => ({ ...p, street: e.target.value }))} className={INPUT} placeholder="Rua" />
                  </div>
                  <div>
                    <label className="text-xs text-white/50 uppercase tracking-wider mb-1 block">Número *</label>
                    <input value={form.number} onChange={e => setForm(p => ({ ...p, number: e.target.value }))} className={INPUT} placeholder="123" />
                  </div>
                  <div>
